...
<div class="buttonsContainer">
    <button @click="backToClientList" type="button" class="next">Powrót</button>  
    <button @click="sendClient" type="button" class="next">Send</button>    
</div>

 </div> 
</div>
   
    <div>{{ this.$store.state.client }}</div>
...
export default ({
 ...
 mounted() {
      fetch('https://nwhwindows-default-rtdb.europe-west1.firebasedatabase.app/test.json').then((response) => {
        if (response.ok){
 ...
 methods: {
    sendClient() {
        ...
        let duplicatedClient = this.clients.find(client => client.firstname === name && client.surname === lastname && client.tel === tel);
        //console.log(duplicatedClient.firstname);

        if(duplicatedClient == null) {
            console.log("send client do database");
            
            fetch('https://nwhwindows-default-rtdb.europe-west1.firebasedatabase.app/test.json', {
                method: 'POST',
                headers : {
